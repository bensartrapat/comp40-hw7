...
umMem_T program_init(FILE *input) 
{
        ...
        /* initialize the unmapStack */
         memory->stackMax = HINT;
         memory->unmapStack = calloc(memory->stackMax, sizeof(uint32_t));
         memory->stackTop = ZERO;
...
static inline int get_new_ID(umMem_T memory)
{
        ...
        if (memory->stackTop == 0) {
                ...
                newID = memory->unmapStack[memory->stackTop];
                memory->stackTop--;
        }
        return newID;
}
...
static inline void store_old_ID(umMem_T memory, int segID)
{
         int newTop = memory->stackTop + 1;
         if (newTop >= memory->stackMax) {
                 int newMax = memory->stackMax * 2;
                 uint32_t* newStack = calloc(newMax, sizeof(uint32_t));
                 for (int i = 1; i<= memory->stackTop; i++) {
                         newStack[i] = memory->unmapStack[i];
                 }
                 free(memory->unmapStack);
                 memory->unmapStack = newStack;
                 memory->stackMax = newMax;
         } 
         memory->unmapStack[newTop] = segID;
         memory->stackTop = newTop;
}
